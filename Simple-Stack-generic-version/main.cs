...
class Stack<T> {
  ...
  public T pop() {
    if (top == -1)
      return default(T); // (T)Convert.ChangeType(value, typeof(T)); 
    Count--;
    return data[top--];
  }
}
...
class MainClass {
  public static void Main (string[] args) {
    ...
    Console.WriteLine(s.Count == 0);
    Console.WriteLine(s.pop() == default(int));     // empty case, default(int) == 0
    Console.WriteLine(s.pop() == 0);
    ...
    Console.WriteLine(s2.Count == 0);
    Console.WriteLine(s2.pop() == default(string));     // empty case, default(string) == null
    Console.WriteLine(s2.pop() == null);
